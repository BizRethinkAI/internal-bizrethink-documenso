  ...
  const padded =
    padding > 0
      ? sharp(svg, { density: 600 })
          .resize(width - padding * 2, height - padding * 2, { fit: 'contain', background: { r: 255, g: 255, b: 255, alpha: 0 } })
          .extend({ top: padding, bottom: padding, left: padding, right: padding, background: { r: 255, g: 255, b: 255, alpha: 0 } })
      : sharp(svg, { density: 600 }).resize(width, height, { fit: 'contain', background: { r: 255, g: 255, b: 255, alpha: 0 } });
...
async function main() {
  ...
  // Apple touch icon: 180px square with 12% safe-inset margin (Apple spec)
  await renderPng(SYMBOL_SVG, resolve(PUBLIC_DIR, 'apple-touch-icon.png'), 180, 180, 11);
