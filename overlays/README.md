...
| 005 | `005-suppress-license-banner-self-host.patch` | **APPLIED 2026-04-30** | LOW | Adds `UNAUTHORIZED` to the early-return guard in `apps/remix/app/components/general/admin-license-status-banner.tsx`. The licence server returns UNAUTHORIZED for our BIZRETHINK tier (overlay 001 has EE-flagged features), but self-host has no functional EE gate per the paywall audit. Suppresses the cosmetic red banner only; PAST_DUE / EXPIRED still surface if upstream ever sends those. |
