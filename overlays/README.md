...
| 008 | `008-unblock-email-domains-self-host.patch` | **APPLIED 2026-04-30** | LOW | Removes the `if (!IS_BILLING_ENABLED()) return null;` early-return from both `o.$orgUrl.settings.email-domains._index.tsx` and `o.$orgUrl.settings.email-domains.$id.tsx`. On self-host, billing is always disabled but the BIZRETHINK claim has `emailDomains: true` — the early-return rendered a blank page instead of letting the downstream claim-flag branch handle the case correctly. Phase A blocker: caught when the user opened `/o/mfg/settings/email-domains` and saw a blank page. Drops the now-unused `IS_BILLING_ENABLED` import from both files. |
